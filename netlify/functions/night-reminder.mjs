...
export default async (request, context) => {
  try {
    ...
    if (!isScheduled && (!triggerSecret || url.searchParams.get("secret") !== triggerSecret)) {
    ...
    // I used to run a query per user here and it tanked the database.
    // Now we do it all in one bulk query. Much faster, total jugaad.
    const [allToday, [{ total: totalQuestions }], allDone] = await Promise.all([
      sql`
        SELECT user_email, COUNT(*) AS solved_today
        FROM   progress
        WHERE  is_done     = TRUE
    ...
    const todayMap = Object.fromEntries(allToday.map(r => [r.user_email, Number(r.solved_today)]));
    const total    = Number(totalQuestions);
    const doneMap  = Object.fromEntries(allDone.map(r => [r.user_email, Number(r.done)]));

    // This LATERAL join took me hours to write. It picks one random unsolved 
    // problem per user without crashing Postgres. Don't touch this query bhai, it's holding everything together.
    ...
    // ── Building the emails in memory. Zero DB calls in this loop because I learned my lesson. ──
    let sent = 0;
    for (const user of users) {
      const userEmail      = user.email;
      const toEmail        = user.send_to;
      const unsubscribeUrl = `${siteUrl}/.netlify/functions/unsubscribe?email=${encodeURIComponent(toEmail)}`;
      const displayName = user.name || toEmail.split("@")[0];

      const solvedToday      = todayMap[userEmail] || 0;
      const suggestedProblem = solvedToday <= 1 ? (problemMap[userEmail] || null) : null;
...
// Just in time for that late night coding session.
export const config = {
  schedule: "30 16 * * *",
};
