...
import { groupBySections, smoothTransition, lockScroll, unlockScroll } from './utils.js';
...
function _closeAllFilterDropdowns() {
  document.querySelectorAll('.filter-dropdown.open').forEach(d => {
    ...
    // Return any portalled dropdown back to its original parent
    if (d._portalParent) {
      d._portalParent.appendChild(d);
      d._portalParent = null;
    }
  });
  const bd = document.getElementById('filter-bd');
  if (bd) { bd.remove(); unlockScroll(); }
}
...
export function toggleCustomDropdown(id) {
  ...
  if (!wasOpen) {
    if (window.innerWidth <= 768) {
      // Portal dropdown to <body> so position:fixed resolves to the viewport,
      // not to .controls (which has transform:translateZ(0) as containing block)
      if (el.parentElement !== document.body) {
        el._portalParent = el.parentElement;
      ...
      bd.id = 'filter-bd';
      bd.addEventListener('click', _closeAllFilterDropdowns, { once: true });
...
document.addEventListener('click', (e) => {
  if (!e.target.closest('.filter-dropdown-wrap') && e.target.id !== 'filter-bd') {
    _closeAllFilterDropdowns();
  }
});
