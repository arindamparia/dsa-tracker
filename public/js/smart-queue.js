      ...
      import('./filters.js').then(m => {
        ...
        requestAnimationFrame(() => {
          ...
          setTimeout(() => {
            ...
            // Read position once — avoids forcing layout on every rAF tick
            const _rowPageTop = tr.getBoundingClientRect().top + window.scrollY;
            const _rowH = tr.offsetHeight;
            let tracking = true;
            function track() {
              if (!tracking) return;
              pointer.style.top = (_rowPageTop - window.scrollY + _rowH / 2) + 'px';
