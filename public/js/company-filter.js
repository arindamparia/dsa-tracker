...
export const CompanyFilter = {
  ...
  toggle() {
    ...
    if (this._open) {
      ...
      if (window.innerWidth <= 768) {
        // Mobile: portal dropdown to <body> so position:fixed is relative to viewport
        // (.controls has transform:translateZ(0) which would otherwise trap fixed children
        //  and make top:50%/left:50% relative to the controls bar, not the page centre)
        if (dd.parentElement !== document.body) {
          dd._portalParent = dd.parentElement;
        ...
        bd.id = 'company-bd';
        bd.addEventListener('click', () => CompanyFilter.close(), { once: true });
        document.body.appendChild(bd);
      } else {
        // Desktop: absolute dropdown inside controls — no portal, no scroll lock
        dd.classList.add('open');
        if (window.innerWidth > 768) setTimeout(() => si?.focus({ preventScroll: true }), 50);
  ...
  _cleanup() {
    const bd = document.getElementById('company-bd');
    if (bd) {
      unlockScroll();
      bd.remove();
    }
    // Return dropdown to its original parent after mobile portal
    const dd = document.getElementById('company-dropdown');
    if (dd && dd._portalParent) {
      dd._portalParent.appendChild(dd);
      dd._portalParent = null;
    }
  },
