...
export const UserSettings = {
  open() {
    ...
    if (perfEl) {
      ...
      if (hintEl) {
        hintEl.textContent = !override
          ? `(detected: ${window.__perfTier || 'full'})`
          : '';
      }
    }
  ...
  async save() {
    ...
    const perfChanged    = perfValue !== prevPerf;
    ...
    if (!enabledChanged && !emailChanged) {
      ...
      this.close();
      if (perfChanged) {
        showToast('Performance mode updated — reloading...', 'info');
    ...
    .then(data => {
      if (!data.ok) throw new Error(data.error || 'Save failed');
      if (!perfChanged) showToast('Settings saved ✓', 'success');
    })
    .catch(err => {
